...
class GNN_DATA:
    def __init__(self, ppi_path, exclude_gene_path=None, max_len=2000, skip_head=True, p1_index=0, p2_index=1, label_index=2, graph_undirection=True, bigger_ppi_path=None):
        self.ppi_list = []
        self.ppi_dict = {}
        self.ppi_label_list = []
        self.gene_dict = {}
        ...
        self.node_num = 0
        self.edge_num = 0
        if exclude_gene_path != None:
            with open(exclude_gene_path, 'r') as f:
                ex_gene = json.load(f)
                f.close()
            ex_gene = {p:i for i, p in enumerate(ex_gene)}
        else:
            ex_gene = {}
        ...
        for line in tqdm(open(ppi_path)):
            ...
            if line[p1_index] in ex_gene.keys() or line[p2_index] in ex_gene.keys():
                continue

            # get node and node name
            if line[p1_index] not in self.gene_name.keys():
                self.gene_name[line[p1_index]] = name
                name += 1
            
            if line[p2_index] not in self.gene_name.keys():
                self.gene_name[line[p2_index]] = name
                name += 1
        ...
        for i in tqdm(range(ppi_num)):
            ...
            # print(len(self.gene_name))
            self.ppi_list[i][0] = self.gene_name[seq1_name]
            self.ppi_list[i][1] = self.gene_name[seq2_name]
        ...
        self.node_num = len(self.gene_name)
        self.edge_num = len(self.ppi_list)
    
    def get_gene_aac(self, pseq_path):
        ...
        for idx, line in enumerate(open(self.pseq_path)):
            ...
            if line[0] not in self.pseq_dict.keys():
                ...
                self.gene_len.append(len(line[1]))
            self.gene2id[line[0]] = idx
        
        print("gene num: {}".format(len(self.pseq_dict)))
        print("gene average length: {}".format(np.average(self.gene_len)))
        print("gene max & min length: {}, {}".format(np.max(self.gene_len), np.min(self.gene_len)))
    ...
    def gene2vec(self, embedding_path):
        embeddings = np.load(embedding_path)
        self.pvec_dict = {}
        for p_name in self.pseq_dict.keys():
            temp_embedding = embeddings[self.gene2id[p_name]]
            self.pvec_dict[p_name] = temp_embedding

    def get_feature_origin(self, pseq_path, vec_path):
        self.get_gene_aac(pseq_path)

        # self.vectorize(vec_path)
        self.gene2vec(vec_path)

        self.gene_dict = {}
        for name in tqdm(self.gene_name.keys()):
            self.gene_dict[name] = self.pvec_dict[name]
    ...
    def generate_data(self):
        ...
        self.x = []
        i = 0
        for name in self.gene_name:
            assert self.gene_name[name] == i
            i += 1
            self.x.append(self.gene_dict[name])
