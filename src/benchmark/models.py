...
class BertForOntoGeneContactPrediction(BertPreTrainedModel):
    ...
    def forward(self, input_ids, gene_length, attention_mask=None, labels=None):
        targets = labels
        outputs = self.bert(input_ids)
        # targets
        sequence_output = outputs[0]
        # print(sequence_output.shape)
        output_precition = self.predict(sequence_output, gene_length, targets) + outputs[2:]
...
def load_sgd_optimizer_and_scheduler(model, args, train_dataset):
...
model_mapping = {
    'promotercore': BertForSequenceClassification2,
