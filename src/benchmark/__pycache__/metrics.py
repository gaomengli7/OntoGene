...
def accuracy_score_remote(y_true, y_pred):
    ...
    # f1 = f1_score(labels=y_true, preds=y_pred, average="macro")
    # mcc = matthews_corrcoef(y_true, y_pred)
    # auc = roc_auc_score(y_true, probs, average="macro", multi_class="ovo")
    # return {
    #     # "acc": acc,
    #     # "f1": f1,
    #     # "mcc": mcc,
...
def compute_accuracy_metrics(task_name, preds, labels):
    if task_name == 'promotercore':
        ...
        return {
            ...
            "recall": recall,
            "f1": f1
            # "acc": accuracy_score_remote(preds, labels),
            # "f1": accuracy_score_remote(preds, labels),
            # "mcc": accuracy_score_remote(preds, labels),
            # # "auc": accuracy_score_remote(labels, probs),
            # "precision": accuracy_score_remote(labels, preds),
            # "recall": accuracy_score_remote(preds, labels),
            # "score_top1": accuracy_score_remote(preds, labels),
        }
    else:
        raise KeyError(task_name)
...
compute_metrics_mapping = {
    ...
    # 'function': compute_accuracy_metrics,
